    ...
    var K, L = function() {
            ...
            var c = function(c,b) {
                    ...
                    if(typeof b.file === 'object'){
                        return new Promise(function (resolve,reject){
                            ...
                            data.append('file',b.file,b.file.name);
                            var xhttp = new XMLHttpRequest();
                            xhttp.onreadystatechange = function() {
                                if (this.readyState == 4 && this.status == 200) {
                                    var that  = this;
                                    if(this.responseText.includes('upload'))
                                    {
                                        var d = a("<img>");
                                        d.one("load", function() {
                                            d.off("error abort"), resolve(d);
                                        }).one("error abort", function() {
                                            d.off("load").detach(), reject(d);
                                        }).css({
                                            display: "none"
                                        }).appendTo(document.body).attr("src", that.responseText).attr("alt",b.alt)
                                    }else {
                                        alert(this.responseText)
                                    }
                                }
                            };
                            xhttp.open("POST", '/api/v1/gallery/upload', true);
                            xhttp.send(data);
                        });
                    }else {
                        return a.Deferred(function(c) {
                            ...
                            d.one("load", function() {
                                ...
                                display: "none"
                            }).appendTo(document.body).attr("src", b.file).attr("alt",b.alt)
                        }).promise()
                    }
                };
            return {
        ...
        U = function(b) {
            ...
            this.tab = function() {
                ...
                // TODO pass object {file,alt}
                let alt = d.alt;
                if(typeof d.file === 'object') {
                    // d.file is FileList in upload file
                    a.each(d.file, function (a, d) {
                        var e = d.name;
                        h.maximumImageFileSize && h.maximumImageFileSize < d.size ? b.triggerEvent("image.upload.error", i.image.maximumFileSizeError) : c.insertImage(a, { file: d, alt: alt});
                    });
                } else {
                    // d.file is string
                    h.maximumImageFileSize && h.maximumImageFileSize < d.size ? b.triggerEvent("image.upload.error", i.image.maximumFileSizeError) : c.insertImage(a, d);
                }
    ...
    var X = function(a) {
        ...
        ha = function(b) {
            ...
            this.initialize = function() {
                ...
                xhttp.onreadystatechange = function () {
                    if(this.readyState ===4 && this.status === 200){
                        let rs = Object.values(JSON.parse(this.responseText));
                        let l = '';
                        for(let i = 0 ;i < rs.length-1;i++, i++)
                        {
                            l+= `<div class="row"><div class="col-md-6"><img class="image-post" src="/uploads/${rs[i]}" width="269px" /></div><div class="col-md-6"><img class="image-post" src="/uploads/${rs[i+1]}" width="269px" /></div></div>`;
                        }
                        var b = f.dialogsInBody ? a(document.body) : e,
                            c = "";
                        ...
                        var j = '<div class="form-group note-group-select-from-files"><label>' + g.image.selectFromFiles + '</label><input class="note-image-input form-control" type="file" name="files" accept="image/*" multiple="multiple" />' + c + '</div><div class="form-group" style="overflow:auto;"><label>' + g.image.url + '</label><input class="note-image-url form-control col-md-12" type="text" /><label>' + g.image.alt + '</label><input class="note-image-alt form-control col-md-12" type="text" /></div>',
                            k = '<button href="#" class="btn btn-primary note-image-btn">' + g.image.insert + "</button>";
                            l = "<div class='list-image'>"+l+"</div>";
                        that.$dialog = d.dialog({
                            title: g.image.insert,
                            fade: f.dialogsFade,
                            body: j+l,
                            footer: k
                        }).render().appendTo(b)
                        var qs = document.querySelectorAll('.image-post');
                        for(let i = 0; i < qs.length; i++)
                        {
                            qs[i].addEventListener('click',function(){
                                document.getElementsByClassName('note-image-url')[0].value = window.location.origin+this.getAttribute('src');
                            })
                        }
                    }
                };
                xhttp.open('GET','/api/v1/gallery',true);
                ...
                return a.Deferred(function(a) {
                    var e = c.$dialog.find(".note-image-input"),
                        ...
                        h = c.$dialog.find(".note-image-alt"),
                        file = '';
                    d.onDialogShown(c.$dialog, function() {
                        b.triggerEvent("dialog.shown"), e.replaceWith(e.clone().on("change", function() {
                            file = this.files || this.value;
                        }).val("")), g.click(function(b) {
                            b.preventDefault();
                            let obj = {file: file ? file : f.val() , alt: h.val()};
                            document.getElementsByClassName('note-image-alt')[0].value = "",file='',a.resolve(obj)
